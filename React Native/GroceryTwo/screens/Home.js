...
const Home = ({ navigation }) => {
  ...
  return (
    <ScrollView
      style={{ flex: 1, backgroundColor: "white", paddingHorizontal: 22, paddingTop: 60 }}
      contentContainerStyle={{ paddingBottom: 80 }}  // Adjust the padding to give space for the tab bar
    >
      <Header />
      ...
      <View style={{ marginBottom: 16, flexDirection: "row", gap: 16, flexWrap: 'wrap', justifyContent: 'space-between' }}>
        {
          categories.map(i => (
            <Category key={i.title} img={i.img} title={i?.title} onPress={() => setSelectedCategory(i?.title)} select={selectedCategory === i.title} />
          ))
        }
      </View>
      ...
      <View style={{ marginBottom: 16, flexDirection: "row", gap: 16, flexWrap: 'wrap', justifyContent: 'space-between' }}>
        {
          filteredProducts?.items.map(i => (
            <ProductCard
              ...
              weight={i.weight}
              img={i.img}
              add={() => navigation.navigate('ProductDetail', { item: i, count: 1 })}
              onPress={() => navigation.navigate('ProductDetail', { item: i })}
            />
          ))
        }
      </View>
